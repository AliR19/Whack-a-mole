...
Note: I am using the old board (STM32VLDISCOVERY BOARD WITH THE SOLDERED ENEL384 BOARD) for the project.

1) This Whack-a-mole game consist of 4 LEDs with 4 pushbuttons. 
...
2) To play the game, there are certain steps to be followed:
...
3) Problems Encountered: While in UC2, the startup routine of 4 flashing
   LED should stop when any of the pushbuttons is supposed to be pressed.
   However this was not happening and it was requiring multiple hitting of
   the PUSHBUTTONS before it could enter UC3. To solve it,the polling of
   the input push buttons state was done as a part of the delay subroutine.

   Another problem encountered was the using up of the existing literal poool
   of 4K memory space in the code section. To overcome this problem , "LTORG"
   directive was added to the code at a point where the processor does not run.

   Features failed to impement: None

   Extra features implemented: None

   Possible future expansion: In the near future, code can be added to
...
4) To adjust the game paramters:
